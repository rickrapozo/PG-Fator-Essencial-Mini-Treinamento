...
const benefits = [
  {
    icon: (
      <svg viewBox="0 0 24 24" className="w-8 h-8 md:w-10 md:h-10 fill-gold mb-4 md:mb-5">
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8">
          {benefits.map((b, i) => (
            <div
              key={i}
              className="reveal-element bg-bg-card p-6 md:p-10 border-t-2 border-gold transition-all duration-300 hover:bg-[#1a3523] hover:border-gold-light"
            >
              {b.icon}
              <h3 className="font-display text-lg md:text-xl mb-3 md:mb-4 text-foreground">{b.title}</h3>
              <p className="text-foreground text-sm md:text-base">{b.desc}</p>
            </div>
          ))}
        </div>

        <p className="reveal-element text-center font-accent text-text-secondary mt-8 md:mt-10 text-sm md:text-base">
